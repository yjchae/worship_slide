...
- **악보 가사 추출** — 악보 이미지(또는 PDF)를 넣으면 오선 아래 가사만 읽어 새 곡으로 저장 (음절을 잇는 `-` 는 지우고 앞뒤를 붙임)
...
- **항목별 배경** — 콘티의 특정 항목(헌금송 등)만 다른 배경(색·이미지)으로. 미리보기·발표 화면(macOS/Windows)·PPTX에 모두 적용
...
| 악보 가사 읽기 | Pillow(오선 찾기) → Tesseract OCR(`kor+eng`) |
...
4. **Tesseract OCR** — 악보 가사 추출에만 필요 (선택). 한국어 가사를 읽으려면 `kor` 언어 데이터까지 설치 — macOS `brew install tesseract tesseract-lang`
